...
const Tables = () => {
  ...
  const { user,tipo } = useContext(AuthContext);
    ...
    const [lista, setLista] = useState([{nombre:'Aceptados',valor:4},{nombre:'Rechazados',valor:5},
    {nombre:'En espera',valor:8},{nombre:'En transito',valor:1},{nombre:'Devuelto',valor:6},{nombre:'Entregados',valor:3},
    {nombre:'Por recoger',valor:0},{nombre:'No entregado',valor:7}]);
    ...
    var newdata= []
           ...
           if (tipo === 0  ){
             if(cliente === null || cliente === undefined || cliente === ''){
              ...
              await   fire.firestore().collection("ordenes").where("status","==",cliente)
              .get()
              .then((querySnapshot) => {
                querySnapshot.forEach((doc) => {
              ...
              if(cliente === null || cliente === undefined || cliente === ''){
                ...
                await   fire.firestore().collection("ordenes").where("cliente", "==", uids).where("status","==",cliente)
                .get()
                .then((querySnapshot) => {
                  querySnapshot.forEach((doc) => {
    ...
    <CRow>
        ...
        <CModal 
            ...
            >
              ...
              <CModalBody>
              <h4>¿Estas seguro de Borrar este pedido pedido?</h4>  
